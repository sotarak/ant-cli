...
const GENERATE_TOKENS_SCRIPT =
  ".agent/skills/design-system/scripts/generate-tokens.cjs";
...
function extractColorsFromMarkdown(content) {
  ...
  const quickRefMatch = content.match(
    /Primary Color\s*\|\s*#([A-Fa-f0-9]{6})\s*\(([^)]+)\)/,
  );
  if (quickRefMatch) {
  ...
  const secondaryMatch = content.match(
    /Secondary Color\s*\|\s*#([A-Fa-f0-9]{6})\s*\(([^)]+)\)/,
  );
  if (secondaryMatch) {
    colors.secondary.name = secondaryMatch[2]
      .toLowerCase()
      .replace(/\s+/g, "-");
    colors.secondary.base = `#${secondaryMatch[1]}`;
  }

  const accentMatch = content.match(
    /Accent Color\s*\|\s*#([A-Fa-f0-9]{6})\s*\(([^)]+)\)/,
  );
  if (accentMatch) {
  ...
  const primarySection = content.match(
    /### Primary Colors[\s\S]*?\|[\s\S]*?(?=###|$)/i,
  );
  if (primarySection) {
    const hexMatches = primarySection[0].matchAll(
      /\*\*([^*]+)\*\*\s*\|\s*#([A-Fa-f0-9]{6})/g,
    );
  ...
  const secondarySection = content.match(
  ...
  const accentSection = content.match(
    /### Accent Colors[\s\S]*?\|[\s\S]*?(?=###|$)/i,
  );
  if (accentSection) {
    const hexMatches = accentSection[0].matchAll(
      /\*\*([^*]+)\*\*\s*\|\s*#([A-Fa-f0-9]{6})/g,
    );
    for (const match of hexMatches) {
      const name = match[1].trim().toLowerCase();
...
function generateColorScale(baseHex, darkHex, lightHex) {
  ...
  return {
    ...
    600: {
      $value: darkHex || adjustBrightness(baseHex, -0.15),
      $type: "color",
    },
...
function adjustBrightness(hex, percent) {
  ...
  const g = Math.min(
    255,
    Math.max(0, ((num >> 8) & 0x00ff) + Math.round(255 * percent)),
  );
  const b = Math.min(
    255,
    Math.max(0, (num & 0x0000ff) + Math.round(255 * percent)),
  );
...
function updateDesignTokens(tokens, colors) {
  ...
  if (tokens.semantic?.color) {
    ...
    sem["primary-hover"] = {
      $value: `{primitive.color.${p}.600}`,
      $type: "color",
    };
    sem["primary-active"] = {
      $value: `{primitive.color.${p}.700}`,
      $type: "color",
    };
    sem["primary-light"] = {
      $value: `{primitive.color.${p}.400}`,
      $type: "color",
    };
    sem["primary-lighter"] = {
      $value: `{primitive.color.${p}.100}`,
      $type: "color",
    };
    sem["primary-dark"] = {
      $value: `{primitive.color.${p}.600}`,
    ...
    sem["secondary-hover"] = {
      $value: `{primitive.color.${s}.600}`,
      $type: "color",
    };
    sem["secondary-light"] = {
      $value: `{primitive.color.${s}.300}`,
      $type: "color",
    };
    sem["secondary-dark"] = {
      $value: `{primitive.color.${s}.600}`,
    ...
    sem["accent-hover"] = {
      $value: `{primitive.color.${a}.600}`,
      $type: "color",
    };
    sem["accent-light"] = {
      $value: `{primitive.color.${a}.300}`,
      $type: "color",
    };
...
function main() {
  ...
  console.log(
    `   Secondary: ${colors.secondary.name} (${colors.secondary.base})`,
  );
  ...
  if (dryRun) {
    ...
    console.log(
      JSON.stringify(tokens.primitive.color, null, 2).slice(0, 500) + "...",
    );
  ...
  if (fs.existsSync(generateScript)) {
    try {
      execSync(
        `node ${generateScript} --config ${DESIGN_TOKENS_JSON} -o ${DESIGN_TOKENS_CSS}`,
        {
          cwd: process.cwd(),
          stdio: "inherit",
        },
